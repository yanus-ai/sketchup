# YanusConnectorSU/web_connect.rb
# Handle API and Token for the YanusConnector plugin
...
module YanusConnector
  class APIConnect
    ...
    def export_to_api(json_data, &callback)
      ...
      begin
        ...
        if response.code.to_i == 200
          ...
          puts "[YANUS ERROR] HTTP Error #{response.code}: #{response.body}" if response.code.to_i >= 400
        end
      rescue StandardError => e
        puts "[YANUS ERROR] Request Failed: #{e.message}"
      end
    ...
    def set_token(token)
      if token.nil?
        Sketchup.write_default('YanusConnector', 'yanustk', nil)
      else
        store_token(token)
      end
      true
    ...
    def generate_encryption_key
      stored_salt = Sketchup.read_default('YanusConnector', 'encryption_salt', nil)
      ...
      unless stored_salt
        ...
        Sketchup.write_default('YanusConnector', 'encryption_salt', encoded_salt)
        stored_salt = encoded_salt
      end

      salt = Base64.strict_decode64(stored_salt) # Decode salt back to binary
      OpenSSL::PKCS5.pbkdf2_hmac('YanusSecureKey', salt, 100_000, 32, 'sha256') # Use SHA-256 explicitly
    end
    ...
    def store_token(token)
      ...
      Sketchup.write_default('YanusConnector', 'yanustk', encrypted_token)
    end

    # Decrypt the stored token
    def decrypt_token
      encrypted_token = Sketchup.read_default('YanusConnector', 'yanustk', nil)
